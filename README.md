...
| セッション録音 | 講師ロール時、メインルームで自動的に全員の音声を録音 |
| EchoNote 連携 | 終了時に録音を [EchoNote](https://github.com/botarhythm/EchoNote) へ自動送信 → 文字起こし＋AI要約 |
...
| `ECHONOTE_URL_MOTOZAWA` | 元沢講師の EchoNote ベースURL | 任意 |
| `ECHONOTE_TOKEN_MOTOZAWA` | 元沢講師の EchoNote ingest token | 任意 |
| `ECHONOTE_URL_TSUKAKOSHI` | 塚越講師の EchoNote ベースURL | 任意 |
| `ECHONOTE_TOKEN_TSUKAKOSHI` | 塚越講師の EchoNote ingest token | 任意 |

EchoNote 連携は講師ごとに任意設定です。設定しない講師は録音→要約フローが無効化され、「自分だけ退出」での通常終了のみが可能になります。詳細は [EchoNote 連携ドキュメント](./docs/echonote-integration.md) を参照してください。
