...
function Check({ children }: { children: React.ReactNode }) {
  return (
    <p className="mt-4 rounded-lg border border-emerald-900/60 bg-emerald-950/30 px-4 py-3 text-pretty text-sm leading-relaxed text-emerald-200">
  ...
  return (
    <figure className="overflow-x-auto rounded-lg border border-stone-800 bg-stone-900 p-4">
      <svg viewBox="0 0 640 260" className="w-full min-w-[520px]" role="img" aria-label="音声の流れ図">
        ...
        {/* ChatGPT の声 */}
        <text x="8" y="20" className="fill-stone-500 text-[11px]">ChatGPT の声</text>
        ...
        <text x="474" y="100" className="fill-stone-500 text-[10px]">録画・配信・モニタ</text>

        {/* 人間の声 */}
        <text x="8" y="170" className="fill-stone-500 text-[11px]">あなたと相手の声</text>
...
function Box({
  ...
  return (
    <g>
      ...
      <text x={x + w / 2} y={y + 34} textAnchor="middle" className="fill-stone-500 text-[10px]">
        {sub}
      </text>
    </g>
  );
...
function Arrow({ x1, y1, x2, y2 }: { x1: number; y1: number; x2: number; y2: number }) {
  return <line x1={x1} y1={y1} x2={x2} y2={y2} stroke="#78716c" strokeWidth={1.5} markerEnd="url(#arrow)" />;
}
